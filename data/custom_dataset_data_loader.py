...
def CreateDataset(dataroots,dataset_mode='2afc',load_size=64, trainset=False , Nbpatches = 205, src_root=None, root_refPatches=None, root_distPatches=None, target=None):
    dataset = None
...
class CustomDatasetDataLoader(BaseDataLoader):
    ...
    def initialize(self, data_csvfile, trainset=False, Nbpatches=205, dataset_mode='2afc',load_size=64,batch_size=1,serial_batches=True, nThreads=1, pin_memory=True, src_root = None, root_refPatches=None, root_distPatches=None, target=None, **dl_kwargs):
        BaseDataLoader.initialize(self)
        if(not isinstance(data_csvfile,list)):
            data_csvfile = [data_csvfile,]

        self.dataset = CreateDataset(data_csvfile,dataset_mode=dataset_mode,load_size=load_size, trainset=trainset, Nbpatches=Nbpatches, src_root=src_root, root_refPatches=root_refPatches, root_distPatches=root_distPatches, target=target)
